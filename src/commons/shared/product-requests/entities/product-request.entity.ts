...
export class ProductRequest {
  ...
  @ApiProperty({ example: 10000, description: '승인 사용자 ID' })
  @Column({ name: 'approve_user_id' })
  approveUserId: number;
  
  @ApiProperty({ example: false, description: '상품 요청 승인 여부' })
  @Column({ name: 'is_approve', default: false })
  isApprove: boolean;
  ...
  @ApiProperty({ example: '승인대기', description: '상품 등록 상태' })
  @Column()
  status: string;
